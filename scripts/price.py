...
def insert_price(code, last_finish, start, finish, highest, lowest, diff_percentage):
    query_sql = "insert into company(id, code, last_finish, start , finish, highest, lowest, diff_percentage, date) " \
                "VALUES (s%,s%,s%)"
    date = time.strftime("%Y-%m-%d", time.time())
    args = (code + str(time.time()), code, last_finish, start, finish, highest, lowest, diff_percentage, date)
